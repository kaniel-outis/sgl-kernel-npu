...
std::tuple<at::Tensor, std::optional<EventHandle>, std::optional<std::function<void()>>, at::Tensor>
                       ...
                       int64_t num_max_dispatch_tokens_per_rank, int64_t num_experts, bool use_fp8)
{
    ...
    if (expertIds.size(0) < PADDING_SIZE) {
        ...
        for (int i = 0; i < this->padding_cnt; i++) {
            at::Tensor tmp_x = torch::ones({1, x.size(1)}, x.options());
            at::Tensor tmp_idx = torch::randperm(num_experts, expertIds.options()).slice(0, 0, topk).reshape({1, topk});
            x_blocks.emplace_back(tmp_x);
    ...
    std::optional<EventHandle> event;
    return {output, event, std::function<void()>([] {}), ep_recv_count};
}
}  // namespace deep_ep
