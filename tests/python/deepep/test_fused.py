...
def reshape_fusion_gmm_weight(weight, dim):
    ...
    weight = weight.view(
        *original_shape[:dim], 2, 32, 64, *original_shape[dim + 1 :]
    )
...
def init_fused_weights_int8(w13_weight, w13_weight_scale, w2_weight, w2_weight_scale,
    device="npu", block_m: int = 16, block_n: int = 16):
    ...
    #     .transpose(1, 2).contiguous()
    # )
    # w13 = torch_npu.npu_format_cast(w13, 2)
    # w13.add_(0)  # 强制触发计算
    ...
    # -------- w2_weight --------
    w2 = w2_weight.data
    print(f"w2 weight: {w2.shape}")

    w2 = torch_npu.npu_format_cast(w2, 29)  # 直接存NZ
    w2_int8_nz = torch.nn.Parameter(w2, requires_grad=False)
    # -------- w13_weight_scale --------

    w13_scale = w13_weight_scale.data.squeeze(-1).contiguous()
    w13_scale = permute_weight(w13_scale, 128)  # 关键的一步
    w13_scale_o = torch.nn.Parameter(w13_scale, requires_grad=False)

    # -------- w2_weight_scale --------
    w2_scale = w2_weight_scale.data.squeeze(-1).contiguous()
    w2_scale_o = torch.nn.Parameter(w2_scale, requires_grad=False)


    return w13_int8_nz, w13_scale_o, w2_int8_nz, w2_scale_o

def make_uniform_topk_idx(num_tokens: int, num_experts: int, num_ranks: int, num_topk: int, device="npu"):
    ...
    for t in range(num_tokens):
        for k in range(num_topk):
            ...
            # 在该 rank 的 expert 范围内取一个 expert（这里简单取第一个，或者轮转）
...
def baseline_test(buffer, x, topk_idx, num_tokens, num_experts, cumulative_local_expert_recv_stats,
    return_recv_hook, w13, w13_scale, w2, w2_scale, topk_weights):
...
def test(
    ...
    if args.active_ranks:
        ...
        # 替换生成方式为均匀分布
        # topk_idx = make_uniform_topk_idx(num_tokens, num_experts, num_ranks, num_topk, device="npu")

    w13_weight, w13_weight_scale, w2_weight, w2_weight_scale = init_base_weights()
    w13, w13_scale, w2, w2_scale = init_baseline_weights(w13_weight.clone().detach(), w13_weight_scale.clone().detach(), w2_weight.clone().detach(), w2_weight_scale.clone().detach())
    w13_f, w13s_f, w2_f, w2s_f = init_fused_weights_int8(w13_weight.clone().detach(), w13_weight_scale.clone().detach(), w2_weight.clone().detach(), w2_weight_scale.clone().detach())
    ...
    # 每个 rank 收到的 token 数量
    tokens_per_rank = torch.zeros(num_ranks, dtype=torch.int64, device="npu")
    for r in range(num_ranks):
        expert_start, expert_end = r * (num_experts // num_ranks), (r + 1) * (num_experts // num_ranks)
        tokens_per_rank[r] = ((topk_idx >= expert_start) & (topk_idx < expert_end)).sum()
    ...
    hidden_states = x
    baseline_output = baseline_test(buffer2, x, topk_idx, num_tokens, num_experts,
        cumulative_local_expert_recv_stats,
        return_recv_hook, w13, w13_scale, w2, w2_scale, topk_weights)

    hidden_states, event, hook = buffer.fused_deep_moe(x, topk_idx, topk_weights,
                                                        w13_f, w13s_f, w2_f, w2s_f,
                                                        num_tokens, num_experts, use_fp8=True)
    ...
    # if rank == 0:
    print(f"[Rank {rank}] {x.dtype=} fused_output:{fused_output} {fused_output.dtype=}, baseline_output:{baseline_output} {baseline_output.dtype=}, fused_output_avg:{fused_output_avg}, baseline_output_avg:{baseline_output_avg}, Max diff between fused and baseline: {diff}, avg_diss: {diff_avg}")
    assert diff_avg < 1e-4, f"[Rank {rank}] Mismatch detected! diff={diff}"
